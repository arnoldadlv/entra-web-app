...
export async function GET(request: NextRequest) {
	...
	if (!domainName || !/^(?:[a-z0-9](?:[a-z0-9-]{0,61}[a-z0-9])?\.)+[a-z0-9][a-z0-9-]{0,61}[a-z0-9]$/i.test(domainName)) {
		return NextResponse.json({ message: "Invalid tenant domain format" }, { status: 400 });
	}
