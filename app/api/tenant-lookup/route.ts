...
export async function GET(request: NextRequest) {
	...
	if (!tenantId) {
		return NextResponse.json({ message: "Missing tenant ID parameter" }, { status: 400 });
	}
	...
	try {
		...
		if (response.cloud === "Commercial" && response.data) {
			return NextResponse.json({ displayName: response.data?.displayName, defaultDomainName: response.data?.defaultDomainName, tenantId: response.data?.tenantId, cloud: response.cloud });
		}
		...
		return NextResponse.json({ message: error instanceof Error ? error.message : "API request failed" }, { status: 500 });
	}
}
